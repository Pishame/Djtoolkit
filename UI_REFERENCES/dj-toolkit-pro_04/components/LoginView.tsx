...
const LoginView: React.FC<LoginViewProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark flex flex-col items-center justify-start py-12 p-6 relative overflow-y-auto custom-scrollbar">
      {/* Background Accents */}
      ...
      >
        <div className="bg-white dark:bg-surface-dark border border-slate-200 dark:border-white/5 rounded-[3rem] p-10 shadow-2xl space-y-8 relative">
          {isRegistering && (
            <button 
              onClick={() => setIsRegistering(false)}
              className="absolute top-8 left-8 flex items-center gap-2 text-slate-400 hover:text-primary transition-all group"
              title="Back to Login"
            >
              <div className="w-8 h-8 flex items-center justify-center rounded-xl bg-slate-50 dark:bg-white/5 group-hover:bg-primary/10 transition-all">
                <span className="material-symbols-outlined text-sm group-hover:-translate-x-1 transition-transform">arrow_back</span>
              </div>
              <span className="text-[9px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">Back to Login</span>
            </button>
          )}
