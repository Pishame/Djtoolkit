...
import { NavItem, AppSettings, ActivityJob } from '../types';
...
const MP4GeneratorModal: React.FC<{ isOpen: boolean; onClose: () => void; onStart: (name: string) => void; performanceMode?: boolean; settings: AppSettings }> = ({ isOpen, onClose, onStart, performanceMode, settings }) => {
  const [files, setFiles] = useState<string[]>([]);
...
const MP4ConcatenatorModal: React.FC<{ isOpen: boolean; onClose: () => void; onStart: (name: string) => void; performanceMode?: boolean; settings: AppSettings }> = ({ isOpen, onClose, onStart, performanceMode, settings }) => {
  const [files, setFiles] = useState<string[]>([]);
...
const CopyrightView: React.FC<{ performanceMode?: boolean; onAddTask?: (name: string) => void; settings: AppSettings }> = ({ performanceMode, onAddTask, settings }) => {
            ...
            <div className="space-y-4">
              <div className="p-5 bg-slate-50 dark:bg-white/[0.02] rounded-2xl border border-slate-100 dark:border-white/5 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-black dark:text-white">1,248</p>
                </div>
...
const HistoryView: React.FC = () => {
  const [historyItems] = useState([
    { id: '1', name: 'Euphoric_Daze_Master.wav', type: 'Downloaded', date: '2023-11-01 10:20', size: '48.2MB', status: 'completed' },
    { id: '2', name: 'Deep_Loop_Techno.mp3', type: 'Analyzed', date: '2023-10-28 15:45', size: '12.4MB', status: 'completed' },
    { id: '3', name: 'Vocals_Sunset_Drive.wav', type: 'Separated', date: '2023-10-27 09:12', size: '35.1MB', status: 'completed' },
    { id: '4', name: 'Drum_Loops_V2.zip', type: 'Converted', date: '2023-10-26 18:30', size: '156.0MB', status: 'completed' },
    { id: '5', name: 'Bass_Isolate_01.flac', type: 'Separated', date: '2023-10-25 11:05', size: '22.8MB', status: 'failed' },
  ]);
    ...
    <div className="p-8 lg:p-12 animate-[fadeIn_0.5s_ease-out] max-w-[1400px] mx-auto pb-24">
      <div className="space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
           {[
             { label: 'Operations', value: '1,428', icon: 'sync_alt', color: 'text-primary' },
...
const VideoModal: React.FC<{ 
  ...
  settings: AppSettings; 
  type: 'youtube' | 'tiktok'
}> = ({ isOpen, onClose, onDownload, settings, type }) => {
  const [url, setUrl] = useState('');
  const [format, setFormat] = useState('MP3');
  const [quality, setQuality] = useState('320kbps');
  const brandColor = type === 'youtube' ? 'bg-red-600' : 'bg-black';
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-xl" onClick={onClose}></div>
      <div className="relative bg-white dark:bg-surface-dark w-full max-w-xl rounded-[3rem] shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden animate-[modalScaleUp_0.3s_ease-out]">
        <div className={`px-10 pt-12 pb-8 ${brandColor} relative overflow-hidden`}>
          ...
          <div className="relative z-10 flex items-center gap-6">
            <div className="w-16 h-16 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center text-white shadow-xl">
              <span className="material-symbols-outlined text-4xl">{type === 'youtube' ? 'play_arrow' : 'music_video'}</span>
            </div>
            <div>
              <h3 className="text-3xl font-black uppercase tracking-tighter text-white">{type} Grabber</h3>
              <p className="text-[10px] font-black text-white/60 uppercase tracking-[0.3em]">Neural Asset Extraction Engine</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-10 space-y-10">
          <div className="space-y-4">
            <div className="flex items-center justify-between px-4">
              ...
              <span className="text-[8px] font-black text-primary uppercase tracking-widest">Validated Link Required</span>
            </div>
            <div className="relative group">
              <span className="material-symbols-outlined absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary transition-colors">link</span>
              <input 
                className="w-full bg-slate-50 dark:bg-white/5 border-2 border-transparent focus:border-primary/20 rounded-2xl pl-14 pr-6 py-5 text-sm dark:text-white placeholder:text-slate-500 transition-all outline-none"
                placeholder={`Paste the ${type} link here...`}
                value={url}
          ...
          <div className="grid grid-cols-2 gap-8">
            <div className="space-y-4">
              <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-4">Output Format</label>
              <div className="flex gap-2 p-1.5 bg-slate-100 dark:bg-white/5 rounded-2xl">
                {['MP3', 'VIDEO'].map((f) => (
                  <button
                    ...
                    className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${format === f ? 'bg-white dark:bg-white/10 text-primary shadow-sm' : 'text-slate-400 hover:text-slate-600 dark:hover:text-white'}`}
                  >
                    {f}
                  </button>
                ))}
            ...
            <div className="space-y-4">
              <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-4">Quality Node</label>
              <CustomSelect 
                value={quality}
          ...
          <div className="pt-4">
            <button 
              ...
              className="w-full py-6 bg-primary text-white rounded-[2rem] font-black uppercase tracking-[0.3em] shadow-2xl shadow-primary/30 hover:brightness-110 active:scale-95 transition-all disabled:opacity-50 disabled:grayscale disabled:scale-100 flex items-center justify-center gap-3"
              disabled={!url}
            >
            ...
            <p className="text-center text-[8px] font-black text-slate-400 uppercase tracking-widest mt-6">
              By using this tool you agree to our terms of service and copyright policies.
            </p>
          </div>
        </div>
      </div>
...
const KeyAnalysisModal: React.FC<{ isOpen: boolean; onClose: () => void; onFinished: (data: {key: string, bpm: string}) => void, performanceMode?: boolean }> = ({ isOpen, onClose, onFinished, performanceMode }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-surface-dark w-full max-w-xl rounded-[2.5rem] shadow-2xl border border-slate-200 dark:border-white/10 p-8 overflow-hidden">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-xl font-black uppercase tracking-tighter dark:text-white">Neural Key Analysis</h2>
        </div>
        {!analyzing ? (
          <div onClick={startAnalysis} className="border-2 border-dashed border-slate-200 dark:border-white/10 rounded-[2rem] p-12 text-center hover:bg-primary/5 cursor-pointer">
          ...
          <div className="py-12 text-center space-y-8">
            <div className="relative w-32 h-32 mx-auto">
              <svg className="w-full h-full transform -rotate-90">
                ...
                <circle cx="64" cy="64" r="60" className="stroke-primary fill-none stroke-8 transition-all duration-300" style={{ strokeDasharray: 377, strokeDashoffset: 377 - (377 * progress) / 100 }} />
              </svg>
              <div className="absolute inset-0 flex items-center justify-center text-xl font-black text-primary">{progress}%</div>
            </div>
            <p className="text-[10px] font-black uppercase tracking-widest text-primary animate-pulse">Syncing Neural Nodes...</p>
          </div>
        )}
      </div>
    </div>
...
const StemSeparatorModal: React.FC<{ isOpen: boolean; onClose: () => void; onStart: (name: string) => void, performanceMode?: boolean }> = ({ isOpen, onClose, onStart, performanceMode }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className={`relative bg-white dark:bg-surface-dark w-full max-w-xl rounded-[3rem] shadow-2xl border border-slate-200 dark:border-white/10 p-10 space-y-8 overflow-hidden`}>
        ...
        <div onClick={() => setFile('track_master.wav')} className={`border-2 border-dashed rounded-[2.5rem] p-16 text-center cursor-pointer transition-all ${file ? 'border-primary bg-primary/5' : 'border-slate-200 dark:border-white/10 hover:border-primary/50'}`}>
        ...
        <button 
          onClick={() => { if (file) { onStart(`Separating Stems: ${file}`); onClose(); } }}
          disabled={!file}
...
const MainContent: React.FC<MainContentProps> = React.memo(({ activeTab, onAddTask, isDarkMode, setIsDarkMode, settings, setSettings }) => {
  ...
  const handleDownload = (type: string, format: string, url: string) => {
    if (onAddTask) onAddTask(`Downloading: ${type.toUpperCase()} Asset (${format})`);
  };
  ...
  if (activeTab === NavItem.Copyright) return <CopyrightView performanceMode={settings.isPerformanceMode} onAddTask={onAddTask} settings={settings} />;
  if (activeTab === NavItem.Music) return <KeyDetectView performanceMode={settings.isPerformanceMode} onAddTask={onAddTask} />;
  if (activeTab === NavItem.History) return <HistoryView />;

  return (
    <div className="p-10 max-w-6xl mx-auto space-y-16 animate-[fadeIn_0.5s_ease-out] pb-32">
      <VideoModal type="youtube" isOpen={isYoutubeModalOpen} onClose={() => setIsYoutubeModalOpen(false)} onDownload={handleDownload} settings={settings} />
      <VideoModal type="tiktok" isOpen={isTiktokModalOpen} onClose={() => setIsTiktokModalOpen(false)} onDownload={handleDownload} settings={settings} />
      ...
      <section className="bg-slate-900 p-10 lg:p-14 rounded-[4rem] flex flex-col lg:flex-row items-center justify-between gap-12 relative overflow-hidden shadow-2xl">
         <div className="relative z-10 flex flex-col gap-6">
            <div>
              <span className="text-[10px] font-black text-primary uppercase tracking-[0.5em] mb-1">Station Telemetry</span>
              <h2 className="text-4xl lg:text-5xl font-black text-white uppercase tracking-tighter leading-none">System <br /> Health</h2>
            </div>
            <div className="grid grid-cols-3 gap-6">
               <div className="flex flex-col">
                  <span className="text-[8px] font-black text-white/40 uppercase tracking-widest mb-1">Process Node</span>
                  <span className="text-xl font-black text-emerald-500">ACTIVE</span>
               </div>
               <div className="flex flex-col">
                  <span className="text-[8px] font-black text-white/40 uppercase tracking-widest mb-1">RAM Engine</span>
                  <span className="text-xl font-black text-blue-400">SYNCED</span>
               </div>
               <div className="flex flex-col">
                  <span className="text-[8px] font-black text-white/40 uppercase tracking-widest mb-1">Cache</span>
                  <span className="text-xl font-black text-white">LOW</span>
               </div>
            </div>
         </div>
         <div className="relative z-10 flex-1 max-w-lg w-full">
            <div className={`bg-white/5 border border-white/10 rounded-[2.5rem] p-8 ${settings.isPerformanceMode ? '' : 'backdrop-blur-md'}`}>
               <div className="flex justify-between items-center mb-6">
                  <div className="flex items-center gap-3">
                     <span className="material-symbols-outlined text-primary text-sm">monitor_heart</span>
                     <p className="text-[10px] font-black text-white/50 uppercase tracking-widest">Real-time Spectral Load</p>
                  </div>
               </div>
               ...
               <div className="mt-8 grid grid-cols-2 gap-4">
                  ...
                  <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                     <div className="h-full bg-blue-500 w-1/3 animate-pulse" style={{ animationDelay: '1s' }}></div>
                  </div>
               </div>
            </div>
         </div>
         {!settings.isPerformanceMode && <div className="absolute top-0 right-0 w-96 h-96 bg-primary/20 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/2"></div>}
      </section>
      ...
      <section className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <ToolkitCard icon="movie" title="MP4 Generator" desc="Batch MP3 to Copyright Video" color="bg-primary" onClick={() => setIsMP4ModalOpen(true)} performanceMode={settings.isPerformanceMode} />
          <ToolkitCard icon="merge" title="MP4 Concatenator" desc="Merge MP4s for Long Tests" color="bg-indigo-500" onClick={() => setIsConcatenatorOpen(true)} performanceMode={settings.isPerformanceMode} />
        </div>
      </section>
      ...
      <section className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <ToolkitCard icon="verified" title="License" desc="Copyright Engine Verification" color="bg-slate-700" onClick={() => onAddTask?.('Checking Licenses...')} performanceMode={settings.isPerformanceMode} />
        </div>
      </section>
