...
interface MainContentProps {
  activeTab: NavItem;
  onAddTask?: (name: string, isPlaylist?: boolean) => void;
  isDarkMode?: boolean;
...
const CopyrightView: React.FC<{ performanceMode?: boolean; onAddTask?: (name: string, isPlaylist?: boolean) => void; settings: AppSettings; usageStats: UsageStats }> = ({ performanceMode, onAddTask, settings, usageStats }) => {
      ...
      {isOverLimit && (
        <div className="absolute inset-0 z-50 bg-slate-950/20 backdrop-blur-sm flex items-center justify-center p-8 rounded-[4rem]">
          <div className="bg-white dark:bg-surface-dark p-12 rounded-[3rem] shadow-2xl border border-slate-200 dark:border-white/10 max-w-lg text-center space-y-8 animate-[modalScaleUp_0.3s_ease-out]">
            ...
            <button className="w-full py-5 bg-primary text-white rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-primary/20 hover:brightness-110 active:scale-95 transition-all">
              Upgrade to Premium
            </button>
          </div>
        </div>
...
const KeyAnalysisModal: React.FC<{ isOpen: boolean; onClose: () => void; onFinished: (data: {key: string, bpm: string}) => void, performanceMode?: boolean }> = ({ isOpen, onClose, onFinished, performanceMode }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-surface-dark w-full max-w-xl rounded-[2.5rem] shadow-2xl border border-slate-200 dark:border-white/10 p-8 overflow-hidden max-h-[90vh] overflow-y-auto custom-scrollbar">
...
const StemSeparatorModal: React.FC<{ isOpen: boolean; onClose: () => void; onStart: (name: string, isPlaylist?: boolean) => void, performanceMode?: boolean }> = ({ isOpen, onClose, onStart, performanceMode }) => {
...
const MainContent: React.FC<MainContentProps> = React.memo(({ 
  ...
  if (activeTab === NavItem.Copyright) return <CopyrightView performanceMode={settings.isPerformanceMode} onAddTask={onAddTask} settings={settings} usageStats={usageStats} />;
