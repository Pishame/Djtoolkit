...
const CopyrightView: React.FC<{ performanceMode?: boolean }> = ({ performanceMode }) => {
...
const VideoModal: React.FC<{ 
  ...
  settings: AppSettings; 
  type: 'youtube' | 'tiktok' | 'soundcloud'
}> = ({ isOpen, onClose, onDownload, settings, type }) => {
  const [url, setUrl] = useState('');
  const [format, setFormat] = useState('MP3');
  const [quality, setQuality] = useState('320kbps');
  const brandColor = type === 'youtube' ? 'bg-red-600' : type === 'tiktok' ? 'bg-black' : 'bg-orange-500';
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-surface-dark w-full max-w-xl rounded-[3rem] shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden animate-[modalScaleUp_0.3s_ease-out]">
        <div className="px-10 pt-10 pb-6 border-b border-slate-100 dark:border-white/5">
          <div className="flex items-center gap-5">
            <div className={`w-14 h-14 ${brandColor} rounded-2xl flex items-center justify-center text-white shadow-xl`}>
              <span className="material-symbols-outlined text-3xl">{type === 'soundcloud' ? 'cloud_download' : 'play_arrow'}</span>
            </div>
            <div>
              <h3 className="text-2xl font-black uppercase tracking-tighter dark:text-white">{type} Grabber</h3>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Extract high-fidelity media assets</p>
            </div>
          </div>
        </div>
        <div className="p-10 space-y-8">
          <div className="space-y-4">
            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-4">Source URL</label>
            <input 
              className="w-full bg-slate-50 dark:bg-white/5 border-2 border-transparent focus:border-primary/20 rounded-2xl px-6 py-5 text-sm dark:text-white placeholder:text-slate-500 transition-all outline-none"
              ...
              <div className="flex gap-2 p-1.5 bg-slate-100 dark:bg-white/5 rounded-2xl">
                {['MP3', 'VIDEO'].map((f) => (
                  <button
                    ...
                    className={`flex-1 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${format === f ? 'bg-white dark:bg-white/10 text-primary shadow-sm' : 'text-slate-400 hover:text-slate-600 dark:hover:text-white'}`}
                  >
                    {f}
                  </button>
                ))}
...
const MainContent: React.FC<MainContentProps> = React.memo(({ activeTab, onAddTask, isDarkMode, setIsDarkMode, settings, setSettings }) => {
  ...
  if (activeTab === NavItem.Copyright) return <CopyrightView performanceMode={settings.isPerformanceMode} />;
  ...
  return (
    <div className="p-10 max-w-6xl mx-auto space-y-16 animate-[fadeIn_0.5s_ease-out] pb-32">
      ...
      <VideoModal type="soundcloud" isOpen={isSoundcloudModalOpen} onClose={() => setIsSoundcloudModalOpen(false)} onDownload={handleDownload} settings={settings} />
      ...
      <section className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <ToolkitCard icon="cloud_download" title="SoundCloud" desc="Direct FLAC/MP3 Streaming" color="bg-orange-500" onClick={() => setIsSoundcloudModalOpen(true)} badge="New" performanceMode={settings.isPerformanceMode} />
        </div>
      </section>
      ...
      <section className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <ToolkitCard icon="edit_note" title="Meta Fix" desc="Auto-repair ID3 Tags" color="bg-blue-500" onClick={() => onAddTask?.('Scanning Library Metadata...')} performanceMode={settings.isPerformanceMode} />
          <ToolkitCard icon="swap_horiz" title="Converter" desc="Batch Format Transcoding" color="bg-amber-600" onClick={() => onAddTask?.('Transcoder Engine Ready.')} performanceMode={settings.isPerformanceMode} />
