...
import { AppSettings, NavItem, ActivityJob, Notification } from '../types';
...
const Header: React.FC<HeaderProps> = ({ onSearch, settings, activeTab, onTabChange, jobs, notifications, onLogout, onViewJob, isPremium, user }) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [searchFilter, setSearchFilter] = useState<'all' | 'library' | 'history'>('all');
    ...
    <header className="h-16 px-8 flex-shrink-0 flex items-center justify-between border-b border-slate-200 dark:border-white/5 bg-background-light dark:bg-background-dark transition-colors duration-500 z-40">
      ...
      <div className="flex items-center gap-12">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            <h1 className="text-xl font-black tracking-tighter text-slate-900 dark:text-white uppercase leading-none">MyDJToolkit</h1>
            {!isPremium && (
              <span className="text-[7px] font-black text-primary uppercase tracking-[0.2em] mt-0.5">Free Neural Tier</span>
            )}
          </div>
        </div>
        ...
        <nav className="hidden md:flex items-center gap-8">
          <div className="relative">
            <button 
              ...
              className={`text-[11px] font-black uppercase tracking-widest transition-colors ${activeTab === NavItem.Toolkit ? 'text-white' : 'text-slate-400 hover:text-white'}`}
            >
              Toolkit
            </button>
          ...
          <div className="relative">
            <button 
              ...
              className={`text-[11px] font-black uppercase tracking-widest transition-colors ${activeTab === NavItem.Copyright ? 'text-white' : 'text-slate-400 hover:text-white'}`}
            >
              Copyright
            </button>
      ...
      <div className="flex-1 max-w-md mx-8 relative" ref={searchRef}>
        ...
        {showResults && (searchQuery.trim() !== '') && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-surface-dark border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-[modalScaleUp_0.2s_ease-out] z-50">
            {/* Filter Pills */}
            <div className="flex items-center gap-2 p-3 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.01]">
              {[
            ...
            <div className="p-2">
              {filteredResults.length === 0 ? (
                <div className="py-8 text-center">
                  <span className="material-symbols-outlined text-slate-300 dark:text-slate-700 text-3xl mb-2">search_off</span>
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">No results found in {searchFilter}</p>
                </div>
              ) : (
            ...
            <div className="bg-slate-50 dark:bg-white/[0.02] px-4 py-2 border-t border-slate-100 dark:border-white/5">
              <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Showing top results for "{searchQuery}"</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-4">
            ...
            {showNotifications && (
              <div className="absolute top-full right-0 mt-2 w-80 bg-white dark:bg-surface-dark border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-[modalScaleUp_0.2s_ease-out] z-50">
                <div className="px-5 py-4 border-b border-slate-100 dark:border-white/5 flex items-center justify-between">
                  ...
                  <span className="text-[8px] font-black text-primary uppercase tracking-widest bg-primary/10 px-2 py-0.5 rounded-full">{notifications.length} Total</span>
                </div>
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((n) => (
                      <button
                        key={n.id}
                        onClick={() => {
                          if (n.type === 'success' && onViewJob) {
                            onViewJob(n.message.split(' has been')[0]);
                          } else {
                            onTabChange(NavItem.History);
                          }
                          setShowNotifications(false);
                      ...
                      >
                        <div className="flex gap-3">
                          <div className={`w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center ${n.type === 'success' ? 'bg-emerald-500/10 text-emerald-500' : n.type === 'error' ? 'bg-red-500/10 text-red-500' : 'bg-blue-500/10 text-blue-500'}`}>
                            <span className="material-symbols-outlined text-sm">{n.type === 'success' ? 'check_circle' : n.type === 'error' ? 'error' : 'info'}</span>
                          </div>
          ...
          <div className="relative" ref={profileRef}>
            ...
            >
              <img src={user?.user_metadata?.avatar_url || `https://picsum.photos/seed/${user?.id || 'default'}/100/100`} alt="User" className="w-full h-full object-cover" />
            </div>

            {showProfileMenu && (
              <div className="absolute top-full right-0 mt-2 w-56 bg-white dark:bg-surface-dark border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-[modalScaleUp_0.2s_ease-out] z-50">
                <div className="p-4 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.01]">
                  <p className="text-[10px] font-black dark:text-white uppercase tracking-tight truncate">{user?.user_metadata?.full_name || user?.email?.split('@')[0] || 'Neural User'}</p>
                  <p className="text-[8px] font-bold text-slate-500 uppercase tracking-widest truncate">{user?.email || 'Guest Session'}</p>
                </div>
                <div className="p-2">
                  <button 
                    onClick={() => {
