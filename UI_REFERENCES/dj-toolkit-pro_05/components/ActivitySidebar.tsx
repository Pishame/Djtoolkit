                ...
                <div className="min-w-0 flex-1">
                  ...
                  {currentTask.stems && (
                    <div className="mt-4 flex flex-wrap gap-2">
                      {currentTask.stems.map(stem => (
                        <span key={stem} className={`px-2 py-1 text-[8px] font-black uppercase tracking-widest rounded-lg border flex items-center gap-1.5 transition-all duration-500 ${
                          isFinished 
                            ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' 
                            : 'bg-primary/5 text-primary border-primary/10'
                        }`}>
                          <span className={`w-1 h-1 rounded-full ${isFinished ? 'bg-emerald-500' : 'bg-primary animate-pulse'}`}></span>
                          {stem}
                        </span>
                      ))}
                    </div>
