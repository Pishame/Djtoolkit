...
const LoginView: React.FC<LoginViewProps> = ({ onLogin }) => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      alert(error.message || 'Failed to initialize Google login.');
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-2.5">
            ...
            {!isRegistering && (
              <div className="flex items-center justify-between px-1">
                <label className="flex items-center gap-1.5 cursor-pointer group">
                  <input type="checkbox" className="w-2.5 h-2.5 rounded border-slate-300 dark:border-white/10 bg-transparent text-primary focus:ring-primary/20" />
            ...
            {error && (
              <motion.div 
                ...
                className="flex items-center gap-1.5 px-2.5 py-1 bg-red-500/10 border border-red-500/20 rounded-lg"
              >
