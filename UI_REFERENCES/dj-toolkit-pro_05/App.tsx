...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (params.get('upgrade') === 'success') {
      setUsageStats(prev => ({ ...prev, isPremium: true }));
      // Clean up URL
      ...
      const successNotif: Notification = {
        ...
        message: 'Your Neural Node has been upgraded to Professional tier.',
        time: 'Just now',
        type: 'success'
      };
      setNotifications(prev => [successNotif, ...prev]);
      setActiveToast(successNotif);
    }
  }, []);
  ...
  const handleAddTask = useCallback((name: string, isPlaylist?: boolean, stems?: string[]) => {
    setCurrentTask({
      name,
      progress: 0,
    ...
    const interval = setInterval(() => {
      ...
      if (prog >= 100) {
        ...
        const newJob: ActivityJob = {
          ...
          parameters: stems ? [...stems, 'Neural V4.2'] : ['Neural V4.2', '44.1kHz', '320kbps']
        };
